/**
 * The CSS named colours (CSS Color Module Level 4), for colour expressions that answer with a name
 * such as "steelblue".
 *
 * Generated once from the color-name package (1.1.4, MIT), which lists the same 148 keywords as the
 * specification; the grey and gray spellings are both there. `transparent` and `currentcolor` are not
 * colours a category can have, and are left out on purpose.
 *
 * It does not parse anything: see color-parse.js.
 *
 * Ported from textview.qs `src/theme/named-colors.js` at df84a5e, unchanged.
 */

/** Name and hex digits, in pairs, alphabetically. */
const TABLE = `
...
/**
 * Turn the table into a lookup.
 *
 * @returns {Map<string, string>} Every name, lower case, mapped to its six hex digits.
 */
function readTable() {
    const words = TABLE.trim().split(/\s+/);
    const colors = new Map();
    for (let index = 0; index < words.length; index += 2)
        colors.set(words[index], words[index + 1]);
    return colors;
}

/** Every named colour, lower case, mapped to its six hex digits. */
export const NAMED_COLORS = readTable();
